...
# pragma omp parallel for
    ...
    for (i = 0; i < nelem; i++)
    {
        ...
        for (k = 0; k < NumSpc; k++)
        {
            NV_Ith(CV_Y, SOIL_SOLUTE(i, k)) = elem[i].chms.t_mole[k];

            elem[i].chmf.react[k] = 0.0;

#if defined(_FBR_)
            NV_Ith(CV_Y, GEOL_SOLUTE(i, k)) = elem[i].chms_geol.t_mole[k];
...
# pragma omp parallel for
    ...
    for (i = 0; i < nriver; i++)
    {
        ...
        for (k = 0; k < NumSpc; k++)
        {
            NV_Ith(CV_Y, RIVER_SOLUTE(i, k)) = river[i].chms.t_mole[k];
        }
    }
}
